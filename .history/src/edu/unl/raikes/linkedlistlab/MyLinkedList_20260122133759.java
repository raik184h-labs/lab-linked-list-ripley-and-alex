...
public class MyLinkedList<E> implements List<E> {
	...
	public int indexOf(Object target) {
		...
		while (true) {
			if (get(i) == null || get(i).equals(target)) {
				return i;
			}
			i++;
			if (i == this.size()) {
	...
	public E remove(int index) {
		if (index == -1) {
			...
			Node tempHead = head;
			head = head.next;
			return tempHead.cargo;
		} else {
			Node tempHead = getNode(index);
			Node prevHead = getNode(index - 1);
	...
	public List<E> subList(int fromIndex, int toIndex) {
		...
		newList.head = this.getNode(fromIndex);
		newList.this.getNode(toIndex) = null;
